...
public:
	...
	//sau
	// G = abs(Gx)+abs(Gy)
	static void filterNormalSobel(cv::Vec4f* normalMeasureData, cv::Vec4b* normalMeasureProcessedData, int width, int height);

	//Segmentare pe baza de muchii
	static void filterDepthSobel(cv::Vec4b* depthData, cv::Vec4b* depthProcessedData, int width, int height);
	static void filterCombinedSobel(uchar* grayscaleProcessedData, cv::Vec4b* depthProcessedData, cv::Vec4b* normalMeasureProcessedData, uchar* combinedSobelImageData, int width, int height);
	static void filterBinarization(uchar* sobelImageData, uchar* binarizedSobelImageData, int width, int height);
	static void filterDilation(uchar* binarizedSobelImageData, uchar* dilatedBinarizedSobelImageData, int width, int height);
	static void edgeSegmentation(uchar* edgeImageData, ushort* regionsData, int width, int height);
	static void regionsToRandomColorImage(ushort* regionsData, cv::Vec4b* segmentedImageRandomColorData, int width, int height);
	//Segmentare planara
	static void planarSegmentation(uchar* grayscaleData, cv::Vec4b* depthData, cv::Vec4f* normalMeasureData, cv::Vec4f* pointCloudData, ushort* regionsData, int width, int height);
	static void regionMerging(ushort* regionsData, int width, int height);
	static void regionsToPropertyImages(ushort* regionsData, uchar* segmentedImageGrayscaleData, cv::Vec4b* segmentedImageDepthData, cv::Vec4b* segmentedImageNormalData, int width, int height);
	static void labelRegions(ushort* regionsData, cv::Vec4b* labeledImageData, int width, int height);
};